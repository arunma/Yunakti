...
public class YModelVisitor extends ASTVisitor implements YModelSource {
	...
	public YModelVisitor(YModel model,
			ICompilationUnit testCaseElementCompilationUnit,
	...
	private void addToModel(String qualifiedName) {
		...
		if (!testCaseConstructed)
			return;
		else if (StringUtils.equals(qualifiedName,
				classUnderTest.getFullyQualifiedName()))
			return;
		else if (StringUtils.equals(qualifiedName, currentClassName))
			return;
	...
	public boolean visit(SimpleName simpleName) {
		...
		if (simpleName.resolveTypeBinding() != null
				&& simpleName.resolveTypeBinding().isClass()) {
	...
	// this method will be invoked for all the method invocations by the
	// testcase
	@Override
	public boolean visit(MethodInvocation node) {
		...
		if (StringUtils.equals(calleeClassName,
			...
			YMethod testMethod = getTestClassMethodIfAvailable(testCaseMethod,
	...
	private YMethod getTestClassMethodIfAvailable(String testCaseMethod,
			String cutClassName, String cutMethodName) {

		/*
		 * if (allClassNames.contains(methodName)){ return null; }
		 */

		YMethod returnYMethod = new YMethod();

		returnYMethod.setParentClass(new YClass(testCase
		...
		if (allTestCaseMethods.contains(testCaseMethod)) {
			...
			for (YMethod eachTestCaseMethod : testCase.getMethods()) {
				if (StringUtils.equalsIgnoreCase(
						eachTestCaseMethod.getMethodName(), testCaseMethod)) {
	...
	private void resolveClassUnderTest(NormalAnnotation node) {
		if (StringUtils.equals(node.getTypeName().getFullyQualifiedName(),
				TEST_CASE_ANNOTATION)) {
			List<MemberValuePair> members = node.values();
			for (MemberValuePair memberValuePair : members) {
				if (StringUtils.equals(memberValuePair.getName().toString(),
						ANNOTATION_PROPERTY_CLASS_UNDER_TEST)) {

					String classUnderTestString = "";

					classUnderTestString = memberValuePair.getValue()
							.toString();
					if (StringUtils.isNotBlank(classUnderTestString)) {
						classUnderTestString = StringUtils.replace(
								classUnderTestString, "\"", "");
					}

					logger.fine("Class Under Test String : "
							+ classUnderTestString);
